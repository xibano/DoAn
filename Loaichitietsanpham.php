...
if (!empty($_POST['id']) && !empty($_POST['loai_ctsp'])) {
  $query = "INSERT INTO loai_chi_tiet_san_pham (id,ten,id_lsp) VALUES (?,?, ?)";
  ...
  $query = "DELETE FROM loai_chi_tiet_san_pham WHERE id= ? ";
...
<html lang-="en">
  <head>
    ...
    <title>Quản lý loại chi tiết sản phẩm</title>
  </head>
  <body>
    <div class="container-fluid">
      ...
      <div class="row">
          ...
          <div class="col-12 col-sm-10 col-md-10 col-lg-10 col-xl-10" style="border: none" >
            ...
            <h1 class="mt-5 mb-4 text-center">Quản lý loại chi tiết sản phẩm</h1>
            <div class="mx-auto" style="width: 85%">
              <form class="mb-3 mt-3 d-inline">
                <a href="Themloaichitietsanpham.php"><button class="btn btn-outline-success" type="button">Thêm</button></a>
                &nbsp; &nbsp;
              </form>

              <form class="d-inline float-end w-50" action="Loaichitietsanpham.php" role="search">
              ...
              <table class="table">
                  ...
                  <tbody>
                    <?php
                      ...
                      $stt = 1;
                      $query = "SELECT loai_chi_tiet_san_pham.id, loai_chi_tiet_san_pham.ten, loai_san_pham.ten ten_loai_san_pham "
                      . "FROM loai_san_pham JOIN loai_chi_tiet_san_pham ON loai_chi_tiet_san_pham.id_lsp = loai_san_pham.id ";
                      if(isset($_GET["search"])) {
                        $query .= " WHERE loai_chi_tiet_san_pham.ten LIKE '%" . $_GET['search']. "%'";
                      }
                      try {
                          $sth = $pdo->query($query);
                          while($row = $sth->fetch()) {
                              ...
                              <td>
                                <a href="Sualoaichitietsanpham.php?id=' . $row['id'] . '" class="text-light">
                                        <button type="button" class="btn btn-danger btn-sm me-1 mb-2" data-mdb-toggle="tooltip" title="edit item">
                                            <i class="fas fa-cog"></i>
                                        </button>
                                    </a>
            
                                    <a href="Loaichitietsanpham.php?id=' . $row['id'] . '" class="text-light">
