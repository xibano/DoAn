...
if (!empty($_POST['ten']) && !empty($_POST['email']) && !empty($_POST['matkhau'])) {
  ...
  $query = "INSERT INTO tai_khoan (ten,email,mat_khau,vai_tro) VALUES (?, ?, ?,?)";
  try {
  ...
  $query="SELECT id FROM tai_khoan WHERE email='" . $_POST['email'] . "';";
  $sth = $pdo->query($query);
  if($row = $sth->fetch()) {
    $idtk = $row['id'];
  }

  $query="INSERT INTO gio_hang VALUES (?,?,?);";
  try {
    $sth = $pdo->prepare($query);
    $sth->execute([
      'GH' . $idtk, 0, $idtk ]);
          ...
          <div class="col-lg-10 col-xl-9 mx-auto">
            <div class="card card-signin flex-row my-5">
              ...
              <div class="card-body">
                ...
                <form class="form-signin" action="Dangky.php" method="POST">
                  <div class="form-label-group">
                    <input type="text" id="inputUserame" class="form-control" placeholder="Username" required autofocus name="ten">
                    <label for="inputUserame">Tên người dùng</label>
                  </div>
                  ...
                  <div class="form-label-group">
                    <input type="password" id="inputConfirmPassword" class="form-control" placeholder="Password" required name="matkhau">
                    <label for="inputConfirmPassword">Nhập lại mật khẩu</label>
                  </div>
                  <form>
                    <button class="btn btn-lg btn-primary btn-block text-uppercase" type="submit" name="submit_dk">Đăng ký</button>
                  </form>
                  
                  <a class="d-block text-center mt-2 small" href="Dangnhap.php">Đăng nhập</a>
